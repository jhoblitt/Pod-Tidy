...
Pod::Tidy - reformat Pod documents without affecting other text or code
...
considers a verbatim block.  As far as C<perldoc> is concerned anything line
that begins with whitespace is in a verbatim block.  While the Pod spec
requires that all blocks are separated by a blank line.
...
If a single black line is added between the two paragraphs as required by

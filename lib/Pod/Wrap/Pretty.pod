...
Please see the Pod of those modules for further API documentation.

This module performs the exact same functions L<Pod::Wrap> I<except> that all
pre-existing newlines are removed prior to line breaking.  This provides
nicely formatted paragraphs similar to L<vim>'s C<gq> command.
